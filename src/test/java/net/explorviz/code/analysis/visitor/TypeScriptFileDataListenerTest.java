...
public class TypeScriptFileDataListenerTest {
  ...
  void testTypeScriptClassParsing() {
    ...
    final TypeScriptFileDataHandler fileDataHandler = tsParserService.parseFileContent(tsCode, fileName, commitSha);

    Assertions.assertNotNull(fileDataHandler);

    final FileData data = fileDataHandler.getProtoBufObject();

    LOGGER.info("TypeScript file parsed: {} classes found", data.getClassesCount());
  ...
  void testJavaScriptClassParsing() {
    ...
    final TypeScriptFileDataHandler fileDataHandler = tsParserService.parseFileContent(jsCode, fileName, commitSha);

    Assertions.assertNotNull(fileDataHandler);

    final FileData data = fileDataHandler.getProtoBufObject();

    LOGGER.info("JavaScript file parsed: {} classes found", data.getClassesCount());
  ...
  void testTypeScriptInterfaceParsing() {
    ...
    LOGGER.info("TypeScript interfaces parsed: {} interfaces found", data.getClassesCount());

    // We should have 2 interfaces: User and Admin
    Assertions.assertTrue(data.getClassesCount() >= 1, "Should have at least 1 interface");

    // Check if we can identify one as an interface
    for (ClassData classData : data.getClassesList()) {
